...
const cappasityRequest = (sku) => {
  const url = `https://3d.cappasity.com/u/${config.ct.cappasityOwner}/${sku}`;
  return axios('https://api.cappasity.com/api/oembed', { params: { url, format: 'json' } });
};
// this script is for better analytic
const cappasityAi = document.createElement('script');
cappasityAi.src = 'https://api.cappasity.com/api/player/cappasity-ai';
